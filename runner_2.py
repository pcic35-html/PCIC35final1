...
if((dir[-1])!=id("/")):
	dir+="/"
sT=int(input("請輸入開始編號"))
cT=int(input("請輸入測資數量"))
...
def runner(i):
	...
	os.system("echo "+str(i)+" | python3 "+dir+"test_make.py > "+ioc(i,".in"))
...
runlist=(threading.Thread(target=runner, args=(i,)) for i in range(sT,sT+cT,1))
for i in runlist:
	i.start()
"""
for i in runlist:
